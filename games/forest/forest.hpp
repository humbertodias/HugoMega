#ifndef FOREST_GAME_HPP
#define FOREST_GAME_HPP
...
void render_forest_bottom() {
    ...
    for (int i = 0; i < game_ctx.lives; i++) {
        if (textures.hugo_lives) {
            if (textures.hugo_lives) textures.hugo_lives->drawAt(0, 32 + i * 40, 188, 1, forest_screen, 1, 0);
        }
    }
    ...
    for (int i = 0; i < 4; i++) {
        if (textures.score_numbers) textures.score_numbers->drawSheetDigit(vals[i], x_score + x_space * i, y_score, forest_screen);
    }
}
...
void render_obstacles() {
    for (int i = 0; i < FOREST_MAX_TIME; i++) {
        ...
        case OBS_ROCK: {
            ...
            if (idx < 0) idx = 0;
            if (textures.rock) textures.rock->drawAt(idx, (int)obstacle_pos, 120, 1, forest_screen, 1, 0);
            break;
        }
        case OBS_TREE: {
            /* Hang still until Hugo is close, then swing once as he approaches. */
            int nframes = (int)textures.tree->getNum();
            if (nframes > 0 && textures.tree &&
                textures.tree->getWidth((Uint32)(nframes - 1)) == 0) {
            ...
            if (textures.lone_tree) textures.lone_tree->drawAt(0, (int)(obstacle_pos - 52), -40, 1, forest_screen, 1, 0);
            if (textures.tree) textures.tree->drawAt(idx, (int)obstacle_pos, 52 + 10, 1, forest_screen, 1, 0);
            break;
        }
        default:
            break;
...
void render_sacks() {
    ...
    for (int i = 0; i < FOREST_MAX_TIME; i++) {
        ...
        if (textures.sack) textures.sack->drawAt(0, (int)(sack_pos - 16), 32, 1, forest_screen, 1, 0);
    }
}
...
void render_leaves() {
    for (int i = 0; i < FOREST_MAX_TIME; i++) {
        ...
        if (game_ctx.leaves[i] == 1) {
            if (textures.leaves2) textures.leaves2->drawAt(0, (int)(leave_pos - 16), -10, 1, forest_screen, 1, 0);
        } else if (game_ctx.leaves[i] == 2) {
            if (textures.leaves1) textures.leaves1->drawAt(0, (int)(leave_pos - 16), -10, 1, forest_screen, 1, 0);
        }
    }
}
...
void render_hugo() {
    ...
    if (game_ctx.arrow_up_focus && game_ctx.hugo_jumping_time >= 0) {
        ...
        int y = (int)(40 - dy);
        int idx = frame % 3;
        if (textures.hugo_jump) textures.hugo_jump->drawAt(idx, hugo_x, y, 1, forest_screen, 1, 0);
    } else if (game_ctx.arrow_down_focus) {
        int y = 105;
        int idx = frame % 8;
        if (textures.hugo_crawl) textures.hugo_crawl->drawAt(idx, hugo_x, y, 1, forest_screen, 1, 0);
    } else {
        int y = 90;
        int idx = frame % 8;
        if (textures.hugo_side) textures.hugo_side->drawAt(idx, hugo_x, y, 1, forest_screen, 1, 0);
    }
}
...
void render_controls() {
    ...
    if (game_ctx.arrow_up_focus) {
        if (textures.arrows) textures.arrows->drawAt(1, 256, 17, 1, forest_screen, 1, 0);
    } else if (textures.arrows) {
        if (textures.arrows) textures.arrows->drawAt(0, 256 + 2, 16 + 3, 1, forest_screen, 1, 0);
    }

    if (game_ctx.arrow_down_focus) {
        if (textures.arrows) textures.arrows->drawAt(3, 256, 54, 1, forest_screen, 1, 0);
    } else if (textures.arrows) {
        if (textures.arrows) textures.arrows->drawAt(2, 256 + 2, 54 + 2, 1, forest_screen, 1, 0);
    }
}
...
static void hugo_debug_bounds(int *out_x, int *out_y, int *out_w, int *out_h)
{
    int hugo_x = HUGO_X_POS;
    double now = get_game_time();
    cgfFile *sprite = textures.hugo_side;
    int y = 90;
...
void render_forest_background() {
    ...
    if (textures.bg_hillsday) {
        if (textures.bg_hillsday) textures.bg_hillsday->drawAt(0, hills_x, 0, 1, forest_screen, 1, 0);
        if (textures.bg_hillsday) textures.bg_hillsday->drawAt(0, hills_x + hills_width, 0, 1, forest_screen, 1, 0);
    }

    // Trees
    if (textures.bg_trees) {
        if (textures.bg_trees) textures.bg_trees->drawAt(0, trees_x, -24, 1, forest_screen, 1, 0);
        if (textures.bg_trees) textures.bg_trees->drawAt(0, trees_x + trees_width, -24, 1, forest_screen, 1, 0);
    }
    ...
    if (textures.bg_ground) {
        for (int i = 0; i < 6; i++) {
            ...
            if (textures.bg_ground) textures.bg_ground->drawAt(0, x, 158, 1, forest_screen, 1, 0);
        }
    }
    ...
    if (textures.grass) {
        for (int i = 0; i < 13; i++) {
            ...
            if (textures.grass) textures.grass->drawAt(0, x, 172, 1, forest_screen, 1, 0);
        }
    }

    // End mountain
    if (textures.end_mountain) {
        if (textures.end_mountain) textures.end_mountain->drawAt(0, mountain_x, -16, 1, forest_screen, 1, 0);
    }
}
...
void render_forest_branch_animation() {
    forest_draw_anim_full(&textures.hugohitlog, get_frame_index_fast(&forest_state_metadata));
}

void render_forest_branch_talking() {
    forest_draw_sync_til(&textures.hugohitlog_talk, textures.sync_hitlog,
                         get_frame_index(&forest_state_metadata));
}

void render_forest_flying_start() {
    forest_draw_anim_full(&textures.catapult_fly, get_frame_index_fast(&forest_state_metadata));
}
...
void render_forest_flying_falling() {
    forest_draw_anim_full(&textures.catapult_fall, get_frame_index_fast(&forest_state_metadata));
}

void render_forest_flying_falling_hang_animation() {
    forest_draw_anim_full(&textures.catapult_hang, get_frame_index_fast(&forest_state_metadata));
}
...
void render_forest_flying_falling_hang_talking() {
    ...
    forest_draw_sync_cgf(&textures.catapult_hangspeak, textures.sync_catapult_hang,
                         get_frame_index(&forest_state_metadata), 115, 117);
}

void render_forest_rock_animation() {
    forest_draw_anim_full(&textures.hugo_lookrock, get_frame_index_fast(&forest_state_metadata));
}

void render_forest_rock_hit_animation() {
    forest_draw_anim_full(&textures.hit_rock, get_frame_index_fast(&forest_state_metadata));
}

void render_forest_rock_talking() {
    forest_draw_sync_cgf(&textures.hugo_telllives, textures.sync_rock, get_frame_index(&forest_state_metadata), 0, 0);
}

void render_forest_trap_animation() {
    forest_draw_anim_full(&textures.hugo_traphurt, get_frame_index(&forest_state_metadata));
}
...
void render_forest_talking_after_hurt() {
    ...
    if (frame < 8 && frame % 4 == 0) {
        if (textures.hugo_hand2) textures.hugo_hand2->drawAt(0, 96, 78, 1, forest_screen, 1, 0);
    } else {
        if (textures.hugo_hand1) textures.hugo_hand1->drawAt(0, 96, 83, 1, forest_screen, 1, 0);
    }
}
...
ForestState process_forest_playing(InputState state) {
    ...
    if (get_frame_index(&forest_state_metadata) % 8 == 0 && !game_ctx.arrow_up_focus) {
        int r = rand() % 5;
...
ForestState process_forest_branch_animation(InputState state) {
    (void)state;
    if (one_shot(&forest_state_metadata, 0.0, 0)) {
        if (FOREST_SOUND_READY(audio.sfx_birds)) forest_play(&audio.sfx_birds);
    }

    if (get_frame_index_fast(&forest_state_metadata) >= forest_anim_frame_count(&textures.hugohitlog)) {
...
ForestState process_forest_flying_start(InputState state) {
    ...
    if (one_shot(&forest_state_metadata, 2.7, 1)) {
        if (FOREST_SOUND_READY(audio.sfx_hugo_screenklir)) forest_play(&audio.sfx_hugo_screenklir);
    }

    if (one_shot(&forest_state_metadata, 2.7, 2)) {
        if (FOREST_SOUND_READY(audio.speak_catapult_hit)) forest_play(&audio.speak_catapult_hit);
    }

    if (get_frame_index_fast(&forest_state_metadata) >= forest_anim_frame_count(&textures.catapult_fly)) {
...
ForestState process_forest_flying_falling(InputState state) {
    ...
    if (get_frame_index_fast(&forest_state_metadata) >= forest_anim_frame_count(&textures.catapult_fall)) {
...
ForestState process_forest_flying_falling_hang_animation(InputState state) {
    ...
    if (get_frame_index_fast(&forest_state_metadata) >= forest_anim_frame_count(&textures.catapult_hang)) {
...
ForestState process_forest_rock_animation(InputState state) {
    (void)state;
    if (get_frame_index_fast(&forest_state_metadata) >= forest_anim_frame_count(&textures.hugo_lookrock)) {
...
ForestState process_forest_rock_hit_animation(InputState state) {
    (void)state;
    if (get_frame_index_fast(&forest_state_metadata) >= forest_anim_frame_count(&textures.hit_rock)) {
...
ForestState process_forest_trap_animation(InputState state) {
    (void)state;
    if (get_frame_index(&forest_state_metadata) >= forest_anim_frame_count(&textures.hugo_traphurt)) {
...
ForestState process_forest_talking_after_hurt(InputState state) {
    ...
    if (one_shot(&forest_state_metadata, 0.0, 1)) {
        if (FOREST_SOUND_READY(audio.sfx_hugo_knock)) forest_play(&audio.sfx_hugo_knock);
        if (game_ctx.lives == 1) {
...
GameState process_forest(InputState state){
    ...
    if (next_state != STATE_FOREST_NONE) {
        ...
        reset_state(&forest_state_metadata);
    }
