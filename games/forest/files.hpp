#ifndef FOREST_FILES_HPP
#define FOREST_FILES_HPP
...
static void forest_anim_init_til(ForestAnim *anim, const char *relpath, int start, int end, const char *sync_relpath)
{
  ...
  anim->is_til = 1;
  anim->til = new tilFile(getFullPath(relpath));
  anim->start = start;
  anim->end = end;
  if (sync_relpath)
    anim->sync = new oosFile(getFullPath(sync_relpath));
}
...
static void forest_anim_init_cgf(ForestAnim *anim, const char *relpath, int start, int end)
{
  memset(anim, 0, sizeof(*anim));
  anim->is_til = 0;
  anim->cgf = new cgfFile(getFullPath(relpath));
  anim->start = start;
  anim->end = end;
}
...
static int forest_load_sound_field(ForestSound *sound, const char *relpath)
{
  ...
  if (forest_load_sound(sound, path.c_str()))
  {
...
#define LOAD_CGF(field, relpath) \
  textures.field = new cgfFile(getFullPath(relpath)); \
  if (!textures.field->getStatus()) { \
    printf("(Forest Error)File not found: %s\n", getFullPath(relpath).c_str()); \
    return 1; \
  }
...
#define LOAD_OOS(field, relpath) \
  textures.field = new oosFile(getFullPath(relpath)); \
  if (!textures.field->getStatus()) { \
    printf("(Forest Error)File not found: %s\n", getFullPath(relpath).c_str()); \
    return 1; \
  }
...
int ForestOpenFiles(void)
{
  ...
  textures.scoreboard = SDL_LoadBMP(getFullPath("ForestData/gfx/SCOREBRD.bmp").c_str());
  if (!textures.scoreboard)
  {
    printf("(Forest Error)File not found: %s\n", getFullPath("ForestData/gfx/SCOREBRD.bmp").c_str());
    return 1;
  }
  {
  ...
  textures.bg_gradient = SDL_LoadBMP(getFullPath("resources/fixed_assets/gradient.bmp").c_str());
  if (!textures.bg_gradient)
    printf("(Forest Warning)File not found: %s\n", getFullPath("resources/fixed_assets/gradient.bmp").c_str());
  ...
  LOAD_OOS(sync_start, "ForestData/Syncs/005-01.oos");
  LOAD_OOS(sync_rock, "ForestData/Syncs/005-02.oos");
  LOAD_OOS(sync_dieonce, "ForestData/Syncs/005-03.oos");
  LOAD_OOS(sync_trap, "ForestData/Syncs/005-04.oos");
  LOAD_OOS(sync_lastlife, "ForestData/Syncs/005-05.oos");
  LOAD_OOS(sync_catapult_talktop, "ForestData/Syncs/005-08.oos");
  LOAD_OOS(sync_catapult_hang, "ForestData/Syncs/005-10.oos");
  LOAD_OOS(sync_hitlog, "ForestData/Syncs/005-11.oos");
  LOAD_OOS(sync_gameover, "ForestData/Syncs/005-12.oos");
  LOAD_OOS(sync_levelcompleted, "ForestData/Syncs/005-13.oos");
...
void ForestUnloadFiles(void)
{
  ...
  delete textures.hugo_side;
  delete textures.hugo_jump;
  delete textures.hugo_crawl;
  forest_free_anim(&textures.hugo_telllives);
  delete textures.hugo_hand1;
  delete textures.hugo_hand2;
  delete textures.catapult;
  delete textures.trap;
  delete textures.rock;
  delete textures.tree;
  delete textures.sack;
  delete textures.lone_tree;
  delete textures.bg_hillsday;
  delete textures.bg_trees;
  delete textures.bg_ground;
  delete textures.grass;
  delete textures.leaves1;
  delete textures.leaves2;
  delete textures.end_mountain;
  delete textures.score_numbers;
  delete textures.hugo_lives;
  delete textures.arrows;
  if (textures.scoreboard)
    SDL_FreeSurface(textures.scoreboard);
  delete textures.cave_hugo_sprite;
  delete textures.cave_score_font;
  ...
  delete textures.sync_start;
  delete textures.sync_rock;
  delete textures.sync_dieonce;
  delete textures.sync_trap;
  delete textures.sync_lastlife;
  delete textures.sync_catapult_talktop;
  delete textures.sync_catapult_hang;
  delete textures.sync_hitlog;
  delete textures.sync_gameover;
  delete textures.sync_levelcompleted;
