#ifndef FOREST_CAVE_HPP
#define FOREST_CAVE_HPP
...
static StateMetadata cave_state_metadata;
...
void render_cave_scoreboard(void) {
    ...
    if (textures.cave_score_font) textures.cave_score_font->drawAt(10, 234, 193, 1, forest_screen, 1, 0);
    forest_draw_rect_fill(272, 201, 38, 28, 11, 11, 11, 255);

    if (textures.cave_score_font) textures.cave_score_font->drawWhiteDigit((Uint32)thousands, x_score + x_space * 0, y_score, forest_screen);
    if (textures.cave_score_font) textures.cave_score_font->drawWhiteDigit((Uint32)hundreds, x_score + x_space * 1, y_score, forest_screen);
    if (textures.cave_score_font) textures.cave_score_font->drawWhiteDigit((Uint32)tens, x_score + x_space * 2, y_score, forest_screen);
    if (textures.cave_score_font) textures.cave_score_font->drawWhiteDigit((Uint32)ones, x_score + x_space * 3, y_score, forest_screen);
}

CaveState process_cave_waiting_before_talking(InputState state) { 
    if (get_state_time(&cave_state_metadata) > 2.5) {
...
CaveState process_cave_talking_before_climb(InputState state) {
    if (one_shot(&cave_state_metadata, 4.0, 0)) {
...
void render_cave_talking_before_climb() {
    forest_draw_sync_til(&textures.cave_talks, textures.cave_talks.sync, get_frame_index(&cave_state_metadata));
    render_cave_scoreboard();
}
...
CaveState process_cave_climbing(InputState state) {
    if (get_frame_index(&cave_state_metadata) >= forest_anim_frame_count(&textures.cave_climbs)) {
        return STATE_CAVE_WAITING_INPUT;
    }

    if(one_shot(&cave_state_metadata, 1.0, 0)){
        forest_play(&audio.cave_nu_kommer_jeg);
    }
    if(one_shot(&cave_state_metadata, 2.0, 1)){
...
void render_cave_climbing() {
    forest_draw_anim_full(&textures.cave_climbs, get_frame_index(&cave_state_metadata));
    render_cave_scoreboard();
}
...
CaveState process_cave_waiting_input(InputState state) {
    if(one_shot(&cave_state_metadata, 0.5, 0)){
...
CaveState process_cave_going_rope(InputState state) {
    ...
    if (one_shot(&cave_state_metadata, sound_times[cave_ctx.cave_selected_rope], 0)) {
        forest_play(&audio.cave_hiv_i_reb);
    }
    
    // Footstep sounds alternating every 0.4 seconds
    if (every(&cave_state_metadata, 0.4, 1, 0.0)) {
        forest_play(&audio.cave_fodtrin1);
    }
    if (every(&cave_state_metadata, 0.4, 2, 0.4)) {
        forest_play(&audio.cave_fodtrin2);
    }
    
    if (get_frame_index(&cave_state_metadata) >= forest_anim_frame_count(get_rope_animation())) {
...
void render_cave_going_rope() {
    forest_draw_anim_full(get_rope_animation(), get_frame_index(&cave_state_metadata));
    render_cave_scoreboard();
}
...
void render_cave_lost() {
    ...
    forest_draw_anim_full(puff_animation, get_frame_index(&cave_state_metadata));
    render_cave_scoreboard();
}

void render_cave_lost_spring() {
    forest_draw_anim_full(&textures.cave_hugo_spring, get_frame_index(&cave_state_metadata));
    render_cave_scoreboard();
}
...
void render_cave_scylla_lost() {
    ...
    forest_draw_anim_full(scylla_animation, get_frame_index(&cave_state_metadata));
    
    // Render Hugo sprite on top
    int hugo_positions[3][2] = {{25, 105}, {97, 100}, {172, 102}};
    if (textures.cave_hugo_sprite) textures.cave_hugo_sprite->drawAt(0, hugo_positions[cave_ctx.cave_selected_rope][0], hugo_positions[cave_ctx.cave_selected_rope][1], 1, forest_screen, 1, 0);
    render_cave_scoreboard();
}

void render_cave_scylla_spring() {
    forest_draw_anim_full(&textures.cave_scylla_spring, get_frame_index(&cave_state_metadata));
    render_cave_scoreboard();
}

void render_cave_family_cage_opens() {
    forest_draw_anim_full(&textures.cave_family_cage, get_frame_index(&cave_state_metadata));
    render_cave_scoreboard();
}

void render_cave_family_happy() {
    forest_draw_anim_full(&textures.cave_happy, get_frame_index(&cave_state_metadata));
    render_cave_scoreboard();
}
...
CaveState process_cave_lost(InputState state) {
    ...
    if (get_frame_index(&cave_state_metadata) >= forest_anim_frame_count(puff_animation)) {
...
CaveState process_cave_lost_spring(InputState state) {
    if (one_shot(&cave_state_metadata, 2.5, 0)) {
        forest_play(&audio.cave_hugo_skyd_ud);
    }
    
    if (get_frame_index(&cave_state_metadata) >= forest_anim_frame_count(&textures.cave_hugo_spring)) {
...
CaveState process_cave_scylla_lost(InputState state) {
    ...
    if (get_frame_index(&cave_state_metadata) >= forest_anim_frame_count(scylla_animation)) {
...
CaveState process_cave_scylla_spring(InputState state) {
    if (one_shot(&cave_state_metadata,1.5, 0)) {
    ...
    if (get_frame_index(&cave_state_metadata) >= forest_anim_frame_count(&textures.cave_scylla_spring)) {
        return STATE_CAVE_FAMILY_CAGE_OPENS;
    }
    return STATE_CAVE_NONE;
}
...
CaveState process_cave_family_cage_opens(InputState state) {
    ...
    if (one_shot(&cave_state_metadata, 1.0, 1)) {
        forest_play(&audio.cave_hugoline_tak);
    }
    
    if (get_frame_index(&cave_state_metadata) >= forest_anim_frame_count(&textures.cave_family_cage)) {
...
CaveState process_cave_family_happy(InputState state) {
    if (get_frame_index(&cave_state_metadata) >= forest_anim_frame_count(&textures.cave_happy)) {
        return STATE_CAVE_END;
    }
    return STATE_CAVE_NONE;
}
...
GameState process_cave(InputState state) {
    ...
    if (next_state != STATE_CAVE_NONE) {
        ...
        reset_state(&cave_state_metadata);
    }
