/* .cbr — ITE RLE-compressed sprite bank (offset table, sparse scanlines). */
class cbrFile {
private:
  struct CbrHead {
...
cbrFile::CbrHead::CbrHead() { std::fill(std::begin(unk), std::end(unk), 0); }

cbrFile::cbrFile(const std::string &fileName, uint32_t myOffset,
                 uint32_t mySize)
    : status(true), filename(fileName), number(0), cbrOffset(myOffset),
      cbrSize(mySize) {
  try {
    ...
    fp.read(reinterpret_cast<char *>(&head.offsetOfOffsets),
            sizeof(head.offsetOfOffsets));
...
bool cbrFile::getStatus() const { return status; }

int cbrFile::getNum() const { return static_cast<int>(number); }

std::vector<unsigned char> cbrFile::getPicture(int num, int &width,
                                               int &height) {
