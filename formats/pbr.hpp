/* .pbr — ITE packed sprite bank (offset table, opcode/delta compression). */
class pbrFile {
private:
  class pbrHead {
  public:
    ...
    pbrHead() { std::fill(std::begin(unk), std::end(unk), 0); }
  };
...
pbrFile::pbrFile(const std::string& fileName, uint32_t myOffset, uint32_t mySize)
    : st(true), fn(fileName), number(0), pbrOffset(myOffset), pbrSize(mySize)
{
    try {
