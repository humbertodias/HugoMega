/* .brs — ITE uncompressed sprite bank (sequential frames, full raster). */
class brsFile {
private:
  struct brsData {
    uint32_t width;
...
brsFile::brsFile(const std::string &fileName, uint32_t myOffset,
                 uint32_t mySize)
    : st(true), fn(fileName), number(0), brsOffset(myOffset), brsSize(mySize) {
  try {
...
bool brsFile::getStatus() const { return st; }

int brsFile::getNum() const { return static_cast<int>(number); }
...
std::vector<uint8_t> brsFile::getPicture(int num, int &width, int &height) {
  ...
  try {
    ...
    int k = 16;
    for (uint32_t i = 0; i < d.height; ++i) {
      for (uint32_t j = 0; j < d.width; ++j) {
